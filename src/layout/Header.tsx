...
function Header() {
  ...
  return (
    <section className="flex-1 flex absolute p-1 top-0 left-0 w-full bg-[#f5d5ce] md:h-12 justify-between items-center">
      ...
      <button onClick={() => setOpen(!open)} className="md:hidden fixed top-1 right-1">
        <Menu size={16} className="bg-transparent" />
      </button>
      {open && (
        <div className="fixed rounded-xl py-1 bg-[#e58b77] mt-2 font-bold text-lg w-1/2 md:w-full md:bg-transparent gap-1 md:relative top-10 md:top-0 right-1 justify-start items-start flex flex-col md:flex-row md:flex-grow md:justify-center md:mt-0 md:items-center md:max-w-3xl text-[#81453a]">
